...
El nivel de las medidas en el caso de pearson se refiere a que ambas variables deben de ser continuas. Si una o ambas variables son ordinales entonces se debe de llevar acabo otro tipo de correlacion como Spearman.
...
```{r}
# lm: linear model y~x
summary(lm(FlujosC$CO2.con~FlujosC$Minute)) # que estoy midiendo? una regresion linear de la liberacion de C02 por min

plot(FlujosC$Minute,FlujosC$CO2.conc)
abline(lm(FlujosC$CO2.con~FlujosC$Minute))
...
```
Si bien el codigo corre de igual manera en este caso ambas son variables dependientes por lo que fueron variables que medimos. Al haberlas medido no seria tan util predecir una usando la otra ya que tendriamos que e igual manera medir una de ellas.
...
```{r}
datos <- data.frame(Peso = c (48,60,72,65,80,88,45), Altura = c(1.65,1.71,1.85,1.78,1.9,1.92,1.52))

cor.test(datos$Altura,datos$Peso,
         method = "pearson")

summary(lm(datos$Altura~datos$Peso))


# y=mx + b
# Altura=0.0087(peso) + 1.18
```
