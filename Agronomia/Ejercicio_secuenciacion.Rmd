...
Nitro <- subset_taxa(Bio, Genus == "Nitrobacter")
```
...
stats <- cbind(metab_stats[,-c(4,10:12)],Norm_vid)
stats <- stats[,-c(10)]

phyla_pvalues <- data.frame(Tratamiento = rep(NA,7), 
                            Muestreo = rep(NA,7))
...
for(i in 3:ncol(stats)){
  ...
  j <- i-2  
  phyla_pvalues$Tratamiento[j] <- T_trat[["p.value"]]
  phyla_pvalues$Muestreo[j] <- S_trat$`Pr(>F)`
}

row.names(phyla_pvalues) <- colnames(stats[3:9])
