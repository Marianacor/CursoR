...
Variables <- read.csv("~/CursoR/CursoRgit/Agronomia/Materiales/Bioensayo NAA_variables.csv")
```
...
ggplot(Promedios)+
  ...
  ggtitle("Bioensayo de NAA a distintas concentraciones")+
  theme(plot.title = element_text(hjust = 0.5, size = 18, color = "black", face = "bold"))+
...
ggplot(Datos, aes(x = Tratamiento, y = Resultado, fill = Variable))+
  theme_classic()+
  ...
  scale_fill_manual(values = c("#260F99", "#7EC3E5"),
                    labels = c("Raíces laterales", "Raíz primaria"),
                    name = "Variables")
```
